-- ─────────────────────────────────────────────────────────────────────────────
-- 20260824000002  Wrap auth.uid() so RLS evaluates it ONCE per statement
-- ─────────────────────────────────────────────────────────────────────────────
--
-- Audit 2026-08-24 (DB-3). Measured live from pg_policies:
--
--   USING clauses containing a bare auth.uid()        84
--   WITH CHECK clauses containing a bare auth.uid()   48
--   distinct policies affected                       113
--   total auth.uid() call sites rewritten            157
--   total policies on schema public                  142
--
-- Written bare, `auth.uid()` is re-evaluated once per CANDIDATE ROW. Wrapped as
-- `(select auth.uid())` the planner hoists it into an InitPlan and evaluates it
-- ONCE for the whole statement. On a sequential scan of a large table that is
-- the difference between N function calls and 1.
--
-- ── SAFE BECAUSE ──────────────────────────────────────────────────────────────
-- (a) SEMANTICS ARE IDENTICAL. auth.uid() reads a GUC (request.jwt.claims) that
--     is fixed for the duration of a statement, so per-row and per-statement
--     evaluation cannot disagree. This is a planner hint, not a logic change.
-- (b) THE PREDICATE TEXT WAS NOT RETYPED. Every statement below was GENERATED
--     from the live catalog (pg_policies) and differs from what is deployed
--     today only by the `(select ...)` wrapper. Nothing was reworded, reordered
--     or "tidied". Verified mechanically before commit:
--        auth.uid() occurrences total ............. 157
--        wrapped as (select auth.uid()) ........... 157
--        left bare ................................   0
--        double-wrapped ...........................   0
-- (c) EVERY ATTRIBUTE IS PRESERVED: policy name, table, PERMISSIVE/RESTRICTIVE
--     (all 113 are permissive; there are no restrictive policies to lose), the
--     command, and the role list (108 -> authenticated, 5 -> public).
-- (d) FOR-ALL policies that carry only a USING clause are re-created with only
--     a USING clause. Postgres derives WITH CHECK from USING in that case, so
--     adding an explicit WITH CHECK would have CHANGED behaviour. It is omitted
--     exactly where the original omits it.
-- (e) DROP + CREATE runs inside one transaction, so no session ever observes a
--     window where a table has lost its policy.
--
-- ── ORDERING ──────────────────────────────────────────────────────────────────
-- Independent of the other 2026-08-24 migrations. Safe in any order.
--
-- Rollback: supabase/rollback/20260824000002_rls_initplan_auth_uid_rollback.sql
-- Verify:   supabase/diagnostics/verify_20260824000002.sql
-- ─────────────────────────────────────────────────────────────────────────────
...
create policy admin_activity_insert on public.admin_activity_log as permissive for insert to authenticated
  with check (((actor_id = (select auth.uid())) AND is_admin()));
...
create policy citizen_details_read_admin_all on public.citizen_details as permissive for select to authenticated
  using ((EXISTS ( SELECT 1
   FROM user_roles ur
  WHERE ((ur.user_id = (select auth.uid())) AND (ur.role_id = 1)))));
...
create policy comment_likes_insert on public.community_comment_likes as permissive for insert to authenticated
  with check (((user_id = (select auth.uid())) AND (is_admin((select auth.uid())) OR is_verified_citizen((select auth.uid())))));
...
create policy official_inserts_own_comment_likes on public.community_comment_likes as permissive for insert to authenticated
  with check (((user_id = (select auth.uid())) AND (current_user_role_id() = ANY (ARRAY[1, 2]))));
...
create policy comments_insert on public.community_comments as permissive for insert to authenticated
  with check (((author_id = (select auth.uid())) AND (is_admin((select auth.uid())) OR is_verified_citizen((select auth.uid())))));
...
create policy community_comments_admin_staff_insert on public.community_comments as permissive for insert to authenticated
  with check (((author_id = (select auth.uid())) AND (is_admin((select auth.uid())) OR is_staff((select auth.uid())))));
...
create policy official_inserts_own_comments on public.community_comments as permissive for insert to authenticated
  with check (((author_id = (select auth.uid())) AND (current_user_role_id() = ANY (ARRAY[1, 2]))));
...
create policy post_images_author_manages on public.community_post_images as permissive for all to authenticated
  ...
  WHERE ((p.id = community_post_images.post_id) AND (p.author_id = (select auth.uid()))))))
...
create policy post_images_read on public.community_post_images as permissive for select to authenticated
  using ((EXISTS ( SELECT 1
   FROM community_posts p
  WHERE ((p.id = community_post_images.post_id) AND ((p.status = 'approved'::text) OR (p.author_id = (select auth.uid())) OR user_has_role((select auth.uid()), 'admin'::text))))));
...
create policy authors_delete_own_pending_posts on public.community_posts as permissive for delete to authenticated
  using (((author_id = (select auth.uid())) AND (status = 'pending_approval'::text)));
...
create policy posts_delete_author_or_admin on public.community_posts as permissive for delete to authenticated
  using (((author_id = (select auth.uid())) OR is_admin((select auth.uid()))));
...
create policy posts_read_approved_own_barangay on public.community_posts as permissive for select to public
  ...
  WHERE (citizen_details.user_id = (select auth.uid()))
 LIMIT 1)))));
...
create policy staff_inserts_own_pending_posts on public.community_posts as permissive for insert to authenticated
  with check (((author_id = (select auth.uid())) AND (current_user_role_id() = ANY (ARRAY[1, 2])) AND (status = 'pending_approval'::text)));
...
create policy "Citizens can delete their own ghost tickets" on public.concern_tickets as permissive for delete to authenticated
  using ((((select auth.uid()) = user_id) AND (is_ghost = true) AND (assigned_staff_id IS NULL) AND (NOT ticket_has_staff_message(id))));
...
create policy "Staff: insert pending event" on public.events as permissive for insert to authenticated
  with check ((((select auth.uid()) = created_by) AND (status = 'pending'::event_status) AND (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'staff'::text))))));
...
create policy "Staff: see own submissions" on public.events as permissive for select to authenticated
  using ((((select auth.uid()) = created_by) AND (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'staff'::text))))));
...
create policy "Staff: update own pending event" on public.events as permissive for update to authenticated
  using ((((select auth.uid()) = created_by) AND (status = 'pending'::event_status) AND (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'staff'::text))))));
...
create policy feedbacks_update_admin on public.feedbacks as permissive for update to authenticated
  using ((EXISTS ( SELECT 1
   FROM user_roles ur
  WHERE ((ur.user_id = (select auth.uid())) AND (ur.role_id = ANY (ARRAY[(1)::bigint, (2)::bigint]))))))
  with check ((EXISTS ( SELECT 1
   FROM user_roles ur
  WHERE ((ur.user_id = (select auth.uid())) AND (ur.role_id = ANY (ARRAY[(1)::bigint, (2)::bigint]))))));
...
create policy staff_admin_send on public.notifications as permissive for insert to authenticated
  with check (((sent_by = (select auth.uid())) AND (is_staff((select auth.uid())) OR is_admin((select auth.uid()))) AND ((COALESCE(target_all, false) = false) OR is_admin((select auth.uid())))));
...
create policy users_read_own on public.notifications as permissive for select to authenticated
  using ((((select auth.uid()) = user_id) AND ((type <> 'staff_message'::text) OR (is_approved = true))));

drop policy if exists "Admin manage profiles" on public.profiles;
create policy "Admin manage profiles" on public.profiles as permissive for all to authenticated
  using ((get_user_role((select auth.uid())) = 'admin'::text))
  with check (((get_user_role((select auth.uid())) = 'admin'::text) AND (NOT ((COALESCE(is_deactivated, false) = true) AND (EXISTS ( SELECT 1
   FROM user_roles ur
...
create policy "Citizens can insert own report media" on public.report_media as permissive for insert to authenticated
  with check ((EXISTS ( SELECT 1
   FROM reports
  WHERE ((reports.id = report_media.report_id) AND (reports.user_id = (select auth.uid()))))));
...
create policy report_notes_staff_insert on public.report_notes as permissive for insert to authenticated
  with check (((author_id = (select auth.uid())) AND (author_role = 'staff'::text) AND staff_can_see_report(report_id)));
...
create policy rrm_insert on public.report_resolution_media as permissive for insert to authenticated
  with check (((uploaded_by = (select auth.uid())) AND (is_admin() OR staff_can_see_report(report_id))));
...
create policy reports_insert_verified_citizen on public.reports as permissive for insert to authenticated
  with check ((((select auth.uid()) = user_id) AND is_verified_citizen()));
...
create policy "Staff can view suggestion media" on public.suggestion_media as permissive for select to authenticated
  using (((EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = ANY (ARRAY['staff'::text, 'admin'::text]))))) AND (EXISTS ( SELECT 1
   FROM suggestions
...
create policy "Ticket participants can read attachments" on public.ticket_attachments as permissive for select to authenticated
  ...
  WHERE ((ct.id = ticket_attachments.ticket_id) AND (ct.user_id = (select auth.uid()))))) OR (EXISTS ( SELECT 1
   FROM concern_tickets ct
  WHERE ((ct.id = ticket_attachments.ticket_id) AND (ct.assigned_staff_id = (select auth.uid()))))) OR (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'admin'::text))))));
...
create policy "Ticket participants can upload attachments" on public.ticket_attachments as permissive for insert to authenticated
  with check (((EXISTS ( SELECT 1
   FROM concern_tickets ct
  WHERE ((ct.id = ticket_attachments.ticket_id) AND ((ct.user_id = (select auth.uid())) OR (ct.assigned_staff_id = (select auth.uid())))))) OR (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'admin'::text))))));
...
create policy "Ticket participants can read messages" on public.ticket_messages as permissive for select to authenticated
  using (((EXISTS ( SELECT 1
   FROM concern_tickets ct
  WHERE ((ct.id = ticket_messages.ticket_id) AND (ct.user_id = (select auth.uid()))))) OR (EXISTS ( SELECT 1
   FROM (user_roles ur
     JOIN roles r ON ((r.id = ur.role_id)))
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = 'admin'::text))))));
...
create policy "Ticket participants can send messages" on public.ticket_messages as permissive for insert to authenticated
  with check ((((select auth.uid()) = sender_id) AND (sender_type = 'citizen'::text) AND ticket_accepts_messages(ticket_id) AND (EXISTS ( SELECT 1
   FROM concern_tickets ct
  WHERE ((ct.id = ticket_messages.ticket_id) AND (ct.user_id = (select auth.uid())))))));
...
create policy staff_writes_department_messages on public.ticket_messages as permissive for insert to authenticated
  with check ((((select auth.uid()) = sender_id) AND (sender_type = 'staff'::text) AND ticket_accepts_messages(ticket_id) AND staff_can_see_ticket(ticket_id)));
...
create policy admin_update_status on public.verification_submissions as permissive for update to authenticated
  ...
  WHERE ((ur.user_id = (select auth.uid())) AND (r.name = ANY (ARRAY['admin'::text, 'staff'::text]))))));
